 ...
 * Skeleton of this code was copied from the CSC316 Workshop 6 guide
 * 
 * @author Dr. King
 * @author Kal Corwin
 *
...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> {
	...
	/** Root of the LinkedBinaryTree */
    private BinaryTreeNode<E> root;
    /** Number of nodes in the LinkedBinaryTree */
    private int size;
    ...
    public static class BinaryTreeNode<E> extends AbstractTreeNode<E> {
        /** Parent of the BinaryTreeNode */
    	private BinaryTreeNode<E> parent;
    	/** Left child of the BinaryTreeNode */
        private BinaryTreeNode<E> left;
        /** Right child of the BinaryTreeNode */
    ...
    public E remove(Position<E> p) {
        ...
        if (right(node) != null) {
        	if (node.equals(root)) {
        		root = validate(right(node));
        		size--;
        		return temp;
        	}
        	if (left(parent(node)).equals(node)) {
        		validate(parent(node)).setLeft(validate(right(node)));
        		validate(right(node)).setParent(validate(parent(node)));
        		size--;
        		return temp;
        	} else {
        		validate(parent(node)).setRight(validate(right(node)));
        		validate(right(node)).setParent(validate(parent(node)));
        		size--;
        		return temp;
        	}
        	...
        	if (node.equals(root)) {
        		root = validate(left(node));
        		size--;
        		return temp;
        	}
        	if (left(parent(node)) != null && left(parent(node)).equals(node)) {
        		validate(parent(node)).setLeft(validate(left(node)));
        		validate(left(node)).setParent(validate(parent(node)));
        		size--;
        		return temp;
        	} else {
        		validate(parent(node)).setRight(validate(left(node)));
        		validate(left(node)).setParent(validate(parent(node)));
        		size--;
        		return temp;
        	}
