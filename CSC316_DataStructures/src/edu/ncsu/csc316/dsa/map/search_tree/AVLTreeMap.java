...
public class AVLTreeMap<K extends Comparable<K>, V> extends BinarySearchTreeMap<K, V> {
    ...
    private void rebalance(Position<Entry<K, V>> p) {
    	...
    	//this bit is added but I think it's necessary. need to initialize newHeight.
    	recomputeHeight(p);
    	int newHeight = getProperty(p); //is this it??
    	BinaryTreeNode<Entry<K, V>> node = (BinaryTreeNode<Entry<K, V>>) p;
    	
    	while ( node != null && oldHeight != newHeight) {
    		oldHeight = getProperty(node); //literally setting old height equal to new height, so if we don't rebalance it won't change??
    		
    		if(!isBalanced(node)) {
    			BinaryTreeNode<Entry<K, V>> child = (BinaryTreeNode<Entry<K, V>>) tallerChild(node);
    			BinaryTreeNode<Entry<K, V>> grandchild = (BinaryTreeNode<Entry<K, V>>) tallerChild(child);
    			
    			node = (BinaryTreeNode<Entry<K, V>>) restructure(grandchild);
    			recomputeHeight(left(node));
    			recomputeHeight(right(node));
    		}
    		recomputeHeight(node);
    		newHeight = getProperty(node);
