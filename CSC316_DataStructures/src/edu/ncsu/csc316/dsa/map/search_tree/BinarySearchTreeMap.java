    ...
    protected static class BalanceableBinaryTree<K, V> extends LinkedBinaryTree<Entry<K, V>> {
        ...
        public void rotate(Position<Entry<K, V>> p) {
        	...
        	if (node.equals(left(parent))) {
        		relink(parent, (BinaryTreeNode<Entry<K, V>>)right(node), true);
        		relink(node, parent, false);
        	} else {
        		relink(parent, (BinaryTreeNode<Entry<K, V>>)left(node), false);
        ...
        public Position<Entry<K, V>> restructure(Position<Entry<K, V>> x) {
        	BinaryTreeNode<Entry<K, V>> node = (BinaryTreeNode<Entry<K, V>>) x;
        	BinaryTreeNode<Entry<K, V>> parent = (BinaryTreeNode<Entry<K, V>>) parent(node);
        	BinaryTreeNode<Entry<K, V>> grandparent = (BinaryTreeNode<Entry<K, V>>) parent(parent);
        	
        	if (node.equals(right(parent)) && parent.equals(right(grandparent)) || node.equals(left(parent)) && parent.equals(left(grandparent))) {
        		rotate(parent);
        		return parent;
        	} else {
        		rotate(node);
        ...
        protected static class BSTNode<E> extends BinaryTreeNode<E> {
            ...
            /**
             * Returns the property of the binary search tree node.
             * 
